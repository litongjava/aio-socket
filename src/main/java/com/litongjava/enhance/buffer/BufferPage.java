...
public final class BufferPage {
  /**
   * 内存页是否处于空闲状态。 当值为true时，表示当前内存页处于空闲状态，可以进行内存回收； 当值为false时，表示当前内存页正在被使用。
   */
  private boolean idle = true;
  /**
   * 待回收的虚拟Buffer队列。 使用线程安全的并发队列存储已被使用完毕但尚未释放的VirtualBuffer对象，
   * 这些对象可以被重新利用，从而减少内存分配和GC压力。
   */
  private final ConcurrentLinkedQueue<VirtualBuffer> cleanBuffers = new ConcurrentLinkedQueue<>();

  /**
   * 标识该内存页是否使用堆外内存（直接缓冲区）。 如果值为 true，表示使用堆外内存，即使用 ByteBuffer.allocateDirect()
   * 分配内存； 如果值为 false，表示使用堆内内存，即使用 ByteBuffer.allocate() 分配内存。 该属性在对象创建后不可修改。
   */
   ...
   * 构造一个内存页对象。
   *
   * @param direct 是否使用堆外内存。 当值为true时，使用ByteBuffer.allocateDirect()分配直接内存；
  ...
  /**
   * 申请指定大小的虚拟内存。 该方法首先尝试从回收队列中获取一个合适大小的VirtualBuffer对象进行复用，
  ...
  /**
   * 回收虚拟内存到回收队列中，使其可以被后续的allocate调用复用。 该方法不会立即释放内存，而是将VirtualBuffer对象放入回收队列中等待复用。
   *
  ...
  /**
   * 尝试回收缓冲区。 该方法由定时任务调用，用于定期检查和回收不再需要的缓冲区。
   * 采用两阶段回收策略：第一次调用时标记内存页为空闲状态，第二次连续调用时才真正回收内存。 这种策略可以避免频繁地分配和释放内存，提高性能。
   */
  ...
  /**
   * 真正回收虚拟缓冲区的底层资源。 对于直接缓冲区(DirectBuffer)，调用其cleaner的clean方法释放本地内存；
  ...
  /**
   * 释放该内存页管理的所有内存资源。 该方法通常在系统关闭或内存页不再使用时调用，用于彻底释放所有资源。
