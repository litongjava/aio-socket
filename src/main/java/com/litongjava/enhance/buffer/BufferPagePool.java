...
public final class BufferPagePool {
  ...
  /**
   * 标识当前JDK版本是否支持直接缓冲区的清理操作。 由于JDK 9及以上版本对sun.nio.ch.DirectBuffer的访问有限制，
   * 因此在高版本JDK中默认不支持直接缓冲区模式。
   */
  private static boolean directSupported = true;

  /**
   * 静态初始化块，用于检测当前JDK版本是否支持直接缓冲区。 通过解析java.version系统属性判断JDK主版本号， 如果主版本号大于8（即JDK
   * 9及以上），则将directSupported设置为false。
   */
  static {
    String version = System.getProperty("java.specification.version");
    // 处理类似"1.8.0_301"和"9+"两种版本格式
    int majorVersion = version.startsWith("1.") ? Integer.parseInt(version.split("\\.")[1])
  ...
  public BufferPagePool(final int pageNum, boolean isDirect) {
    ...
    if (isDirect && !directSupported) {
      isDirect = false;
      System.err.println(
          "The current version of JDK does not support applying for Direct ByteBuffer. Please switch the JDK to 1.8 or upgrade smart-socket to the 1.6.x version.");
  ...
  public VirtualBuffer allocateResposneByThreadId(final int size) {
