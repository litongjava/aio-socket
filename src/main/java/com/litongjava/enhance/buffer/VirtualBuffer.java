 ...
 * 该类实现了对ByteBuffer的池化管理，通过与BufferPage关联， 实现了缓冲区的复用和资源回收，减少内存分配和GC压力。
...
public final class VirtualBuffer {
  ...
  /**
   * 当前虚拟buffer的归属内存页。 如果该值不为null，表示这个VirtualBuffer是由BufferPage创建和管理的，
   * 当不再使用时，会被回收到对应的BufferPage中； 如果该值为null，表示这个VirtualBuffer是通过wrap方法创建的，
   * 不受内存池管理，不会被回收复用。
   */
  private final BufferPage bufferPage;
  /**
   * 用于控制缓冲区清理状态的信号量。 使用信号量确保每个VirtualBuffer只能被清理一次，防止重复清理导致的问题。
   * 初始值为1，表示可以进行一次清理操作； 当clean()方法被调用后，信号量值变为0，表示已经被清理，不能再次清理。
   */
  ...
  /**
   * 构造一个虚拟缓冲区对象。 该构造函数为包级私有，只能由同包中的类调用，通常由BufferPage在分配内存时调用。
   *
  ...
  /**
   * 将现有的ByteBuffer包装成VirtualBuffer。 通过这种方式创建的VirtualBuffer不受内存池管理，不会被回收复用。
  ...
  /**
   * 获取底层的ByteBuffer实例。 调用者可以直接操作返回的ByteBuffer，进行读写等操作。
   * 注意：在使用完毕后，应该调用VirtualBuffer的clean()方法而不是直接释放ByteBuffer， 以确保资源能够正确回收。
   *
  ...
  /**
   * 重置缓冲区状态，准备复用。 该方法会清空底层ByteBuffer的内容，并重置信号量状态，使VirtualBuffer可以再次使用。
  ...
  /**
   * 释放虚拟缓冲区资源。 如果该VirtualBuffer是由BufferPage创建的，则会将其归还给对应的BufferPage进行复用；
   * 如果是通过wrap方法创建的，则不做特殊处理。 该方法使用信号量确保每个VirtualBuffer只能被清理一次，防止重复清理。
   *
  ...
  /**
   * 返回该虚拟缓冲区的字符串表示形式，用于调试和日志记录。 直接返回底层ByteBuffer的字符串表示形式。
   *
